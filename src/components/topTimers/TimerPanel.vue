...
function doReset() {
  lastTimerString.value = extraTimer
    .getTimeValues()
    .toString(['minutes', 'seconds']);
  extraTimer.reset();
}
            ...
            <v-container class="d-flex align-center pa-0 ma-0 w-auto">
              <span class="pr-1"
                ><v-btn
                  ...
                  icon="mdi-refresh"
                  size="x-small"
                  @click.stop="doReset"></v-btn
              ></span>
              <v-btn
                :color="isPaused ? 'error' : 'secondary-darken-1'"
                :icon="isPaused ? 'mdi-play' : 'mdi-pause'"
                size="x-small"
                @click.stop="togglePause"></v-btn>
            </v-container>
          </v-container>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <v-container class="pa-0 ma-0 d-flex justify-space-between">
            <span class="text-body-1">Last Timer:</span>
            <span>{{ lastTimerString }}</span>
          </v-container>
          <span class="text-body-2"
            >The settings for this timer will live here in the future. Just
            dream of it for now. Pretend it rules. Be excited.</span
          >
        </v-expansion-panel-text>
      </v-expansion-panel>
    </v-container>
